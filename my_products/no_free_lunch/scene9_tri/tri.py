...
class Tridiagonal(MovingCameraScene):
    def construct(self):
        ...
        _matrix_lzb_ = VGroup( matrix_l____, matrix_z____, eq____, matrix_b____ ).shift( RIGHT * 0.2222 )
        self.play(
        ...
        self.play(
            tex_a_equals_lu.animate.shift(UP*2.2222 + LEFT*4.4444), tex_column_alu.animate.shift(UP*4.4444 + LEFT*4.4444),
            _matrix_alu_.animate.shift(UP*2.2222 + LEFT*4.4444),
            tex_ux_equals_z.animate.shift(UP*4.4444 + LEFT*4.4444), tex_column_uxz.animate.shift(UP*4.4444 + LEFT*4.4444),
            _matrix_uxz__.animate.shift(UP*4.4444 + LEFT*4.4444),
            tex_lz_equals_b.animate.shift(UP*4.4444 + LEFT*4.4444), tex_column_lzb.animate.shift(UP*4.4444 + LEFT*4.4444),
            _matrix_lzb_.animate.shift(UP*4.4444 + LEFT*4.4444),
        )
        ...
        self.play(Create(diaLine_a))
        self.play(Create(diaLine_l))
        self.play(Create(diaLine_u))
        ...
        tex_aij2 = MathTex(r" l_{ij} = a_{ij} - \sum_{k=1}^{j-1} l_{ik}u_{kj}. \quad (i=i,2,...,n, \, j=1,2,...,i) ").next_to(_matrix_lzb_, DOWN*4.4444)
        ...
        self.play(
            self.camera.frame.animate.set(width = 33.333333).shift( UP*6.666 )
        )
        ...
        self.play(
            self.camera.frame.animate.shift( UP*3.333 ).set(width = 33.333333 )
        )
        
        group_program = VGroup( text_pseudocode, tex_tip_step123, _step1_, _step2_, _step3_, _step4_, _step5_, group_step1, _shabi_, group_step3, tex_step4_1, tex_step5_1, text_output )
        
        self.play(
            group_qwq.animate.shift( LEFT*8.88888888 ),
            group_program.animate.shift( LEFT*8.88888888 )
        )
